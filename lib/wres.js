...
module.exports.render = function(template, theme, info) {
  // Load default template and theme
  if (!template)
    template = fs.readFileSync(path.resolve(__dirname, '../template.jade'));
  if (!theme)
    theme = fs.readFileSync(path.resolve(__dirname, '../themes/lines.css'));

  // Preserve new line characters through Jade, and merge theme
  info = JSON.parse(JSON.stringify(info).replace(/\\n/g, '\\\\n'));
  info.__style = new CleanCSS().minify(theme).styles;
